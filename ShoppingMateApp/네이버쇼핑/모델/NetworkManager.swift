...
class NetworkManager {
    ...
    func shoppingRequest(query: String, display: Int = 30, start: Int = 1, sort: String = "sim", completion: @escaping (Result<(Int, [Item]), Error>) -> Void) {
        ...
        AF.request(url, method: .get, parameters: parameters, headers: headers).validate().responseDecodable(of: Shop.self) { response in
            ...
            case .success(let shop):
                completion(.success((shop.total, shop.items)))
